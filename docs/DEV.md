...
`chime_enabled` is the master mute. `chime_source` is `preset`, `wav` (URL-pull cache), or `uploaded` (files in `/storage/chimes`). `chime_pull_mode` is `once` or `with_rotate`. Filename sanitizing is in `main/chime_name.c` (host-tested). Other boards keep the no-op `board_hal_play_chime*()` stubs. Keep `esp_driver_i2c` on `board_hal` **REQUIRES** (public headers include `driver/i2c_master.h`).
